...
static bool expr_match(const struct chr *chrv, uint32_t n, uint8_t c, bool neg) {
    ...
    for (i=0; i<n; i++) {
        if (c < chrv[i].min) continue;
        if (c > chrv[i].max) continue;
        break;
    }
 ...
 *
 * @return  WSTK_STATUS_SUCCESS or WSTK_STATUS_NOT_FOUND or some error
 *
 * Example:
 *
...
again:
    ...
    for (; *ep; ep++) {
        ...
        /* End of character class */
            ...
            wstk_pl_t lpl, *pl = va_arg(ap, wstk_pl_t *);
            ...
            /* Strip quotes */
            if (qesc && lpl.l > 1 && lpl.p[0] == '"' && lpl.p[lpl.l - 1] == '"') {
...
chr:
        if (n >= ARRAY_SIZE(chrv)) break;
        chrv[n].max = tolower(*ep);
        if (range) range = false;
        else chrv[n].min = tolower(*ep);
        ++n;
    }
out:
    va_end(ap);
    ...
    return(*ep ? WSTK_STATUS_NOT_FOUND : WSTK_STATUS_SUCCESS);
}
