 ...
 *
 * @param ctx       - the context
 * @param digest    - digest buffer (WSTK_MD5_DIGEST_SIZE)
 *
 * @return success or error
 **/
 ...
 * Digest to hex
 *
 * @param hex       - buffer (WSTK_MD5_DIGEST_STRING_SIZE)
...
wstk_status_t wstk_md5_digest2hex(uint8_t *hex, uint8_t *digest) {
    ...
    return wstk_str_bin2hex((char *)hex, digest, WSTK_MD5_DIGEST_SIZE);
}
...
bool wstk_md5_is_equal(uint8_t *dig1, uint8_t *dig2) {
    ...
    if(memcmp(dig1, dig2, WSTK_MD5_DIGEST_SIZE) == 0) {
        return true;
    }
    return false;
}
