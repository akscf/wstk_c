...
#ifdef WSTK_HAVE_KQUEUE
#include	<sys/event.h>
#endif

struct wstk_poll_kqueue_s {
#ifdef WSTK_HAVE_KQUEUE
...
#ifdef WSTK_HAVE_KQUEUE
static void destructor__wstk_poll_kqueue_t(void *data) {
    wstk_poll_kqueue_t *poll = (wstk_poll_kqueue_t *)data;
...
static wstk_status_t poll_socket_add_perform(wstk_poll_kqueue_t *poll, wstk_socket_t *socket) {
    wstk_status_t status = WSTK_STATUS_SUCCESS;
    struct kevent kev[2] = { 0 };
    int err=0, n=0, fd = socket->fd;
    ...
    if(wstk_core_inthash_find(poll->sockets, socket->fd)) {
        ...
        if(status == WSTK_STATUS_SUCCESS) {
            if(socket->pmask & WSTK_POLL_MREAD)  {
                EV_SET(&kev[n++], fd, EVFILT_READ, EV_ADD|EV_CLEAR, 0, 0, socket);
            }
            if(socket->pmask & WSTK_POLL_MWRITE) {
                EV_SET(&kev[n++], fd, EVFILT_WRITE, EV_ADD|EV_CLEAR, 0, 0, socket);
            }
            if(n) {
                if(kevent(poll->kqfd, kev, n, NULL, 0, NULL) < 0) {
                    err = errno;
...
static wstk_status_t poll_socket_del_perform(wstk_poll_kqueue_t *poll, wstk_socket_t *socket) {
    wstk_status_t status = WSTK_STATUS_SUCCESS;
    struct kevent kev[2] = { 0 };
    int fd = socket->fd;
    ...
    if(wstk_core_inthash_delete(poll->sockets, socket->fd)) {
        ...
        kevent(poll->kqfd, kev, 2, NULL, 0, NULL);
    }

    return status;
}

static wstk_status_t poll_deferred_action(wstk_poll_kqueue_t *poll, wstk_socket_t *socket, int act) {
...
wstk_status_t wstk_poll_kqueue_create(wstk_poll_kqueue_t **poll, uint32_t size, uint32_t timeout, uint32_t flags, wstk_poll_handler_t handler, void *udata) {
    wstk_status_t status = WSTK_STATUS_SUCCESS;
    wstk_poll_kqueue_t *pvt = NULL;

    status = wstk_mem_zalloc((void *)&pvt, sizeof(wstk_poll_kqueue_t), destructor__wstk_poll_kqueue_t);
    if(status != WSTK_STATUS_SUCCESS) {
        goto out;
    }
    ...
    pvt->kqfd = kqueue();
    if(pvt->kqfd < 0) {
        log_error("kqueue: err=%d", pvt->kqfd);
...
wstk_status_t wstk_poll_kqueue_polling(wstk_poll_kqueue_t *poll) {
    wstk_status_t status = WSTK_STATUS_SUCCESS;
    struct timespec tv = { 0 }, *tv_ptr = NULL;
    wstk_hash_index_t *hidx = NULL;
    time_t curr_ts = 0;
    ...
    rc = kevent(poll->kqfd, NULL, 0, poll->events, fds, tv_ptr);
    ...
    if(rc > 0) {
        for(int i=0; i < rc; i++) {
            struct kevent *kev = &poll->events[i];
            wstk_socket_t *sock = kev->udata;
            ...
            if(sock) {
                ...
                if(kev->flags & EV_ERROR) {
                    log_error("EV_ERROR: sock=%p, fd=%d", sock, sock->fd);
                }
